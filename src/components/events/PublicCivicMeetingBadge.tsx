...
export function PublicCivicMeetingBadge({ event, candidate }: Props) {
  ...
  const partyPresentation = event ? getPartyMeetingPresentation(event) : null;
  const party = candidate?.partyLabel ?? event?.partyLabel;
  const verified = (candidate?.confidenceScore ?? 0) >= 70;

  if (partyPresentation) {
    return (
      <div className="flex flex-wrap gap-1 mt-1">
        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded ${partyPresentation.badgeClassName}`}>
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-1">
      ...
      {category === "public_party_meeting" && (
        <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-slate-700 text-white">Party committee</span>
      )}
